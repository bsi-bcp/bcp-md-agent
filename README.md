...
#### 云边协同
- **云端配置下发** - 通过华为云 IoTEdge 平台的模块影子机制下发配置
- **边端引擎执行** - 边端接收配置后自动更新数据源和集成任务
- **配置统一管理** - 云端集中管理边缘节点配置，降低运维复杂度
- **双向通信** - 支持云边双向数据传输和状态同步

#### 热部署
- **无需重启** - 配置变更实时生效，无需重启边端服务
- **动态更新**：
  - 数据源配置动态刷新 (`agDataSourceService.updateDS()`)
  - 集成任务配置实时更新 (`agConfigService.updateConfig()`)
  - Cron 表达式动态调整
- **零停机部署** - 保证业务连续性，避免服务中断
- **配置版本管理** - 通过模块影子追踪配置变更历史
